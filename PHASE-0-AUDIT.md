...
**POC catalogue audit (2026-07-15):** The original site's public POC Playground documents descriptions for only five companies: CorrActions, Tactile Mobility, Maradin, Coreteel and Actasys. Its public Portfolio endpoint exposes CorrActions, two SafeMode entries, AutoTrust (under the `siraj` slug) and ThermoTerra; several Portfolio descriptions still contain lorem ipsum. Of the corrected 110 supplied records, 16 records across eight company names can use matched original-site media (the five documented cases plus SafeMode, AutoTrust and ThermoTerra). The remaining 94 records use a clearly non-photographic typographic fallback; no unrelated image is presented as POC evidence.
