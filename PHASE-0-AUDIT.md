...
| F-13 | Supplied list described as "110 POCs" | POC catalogue | The supplied file contains **195 distinct non-empty records and no exact duplicates**. It includes repeated technologies at different partners/sites, demos, Quantum-labelled projects and several ambiguous spellings, so it cannot be reconciled to the confirmed 110 headline without an internal source-of-truth review. | Amir's supplied list, 2026-07-15 | ❓ retain all records; do not change F-08 |
...
**POC catalogue audit (2026-07-15):** The original site's public POC Playground documents descriptions for only five companies: CorrActions, Tactile Mobility, Maradin, Coreteel and Actasys. Its public Portfolio endpoint exposes CorrActions, two SafeMode entries, AutoTrust (under the `siraj` slug) and ThermoTerra; several Portfolio descriptions still contain lorem ipsum. Of the 195 supplied records, 12 records across seven company names can therefore use matched original-site media (the five documented cases plus SafeMode and AutoTrust). The remaining 183 records use a clearly non-photographic typographic fallback; no unrelated image is presented as POC evidence.
